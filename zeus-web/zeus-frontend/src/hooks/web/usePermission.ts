...
import type { UserPermsResponse } from '/@/api/sys/model/userModel';
...
export function usePermission() {
   ...
   */
  function hasPermission(value?: UserPermsResponse | UserPermsResponse[] | string | string[], def = true): boolean {
    ...
    //   }
    //   if (!isArray(value)) {
    //     return userStore.getPermListState?.includes(value as UserPermsResponse);
    //   }
    //   return (intersection(value, userStore.getPermListState) as UserPermsResponse[]).length > 0;
    // }
   ...
   * @param roles
   */
  async function changeRole(roles: UserPermsResponse | UserPermsResponse[]): Promise<void> {
    ...
    userStore.commitPermListState(roles);
    await resume();
  }
