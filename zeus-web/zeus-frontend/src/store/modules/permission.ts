...
class Permission extends VuexModule {
  ...
  async buildRoutesAction(): Promise<AppRouteRecordRaw[]> {
    ...
    let permList = userStore.getPermListState;

    if (!permList){
      await userStore.getUserInfoAction().then(_ =>{
        permList = userStore.getPermListState;
      });
    }

    if (isNullOrUnDef(permList)){
      return [];
    }
    // routes = asyncRoutes;
    ...
    routes = filter(asyncRoutes, (route) => {
      ...
      return perm.displayOperationControl === 2;
    });
