...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    const { serverPort, joinPrefix, joinParamsToUrl, formatDate, joinTime = true } = options;

    if (joinPrefix) {
      config.url = `${prefix}${config.url}`;
    }

    if (serverPort && isString(serverPort)) {
      config.url = `:${serverPort}${config.url}`;
    } else{
      const windowPort = window.location.port;
      if (windowPort){
        config.url = `:${windowPort}${config.url}`;
      }
    }

    config.url = `${window.location.protocol}//${window.location.hostname}${config.url}`;
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    const msg: string = response?.data?.message?.content ?? '';
    ...
    deepMerge(
      {
        ...
        requestOptions: {
          ...
          // 接口地址
          serverPort: globSetting.serverPort,
          //  是否加入时间戳
          joinTime: true,
          // 忽略重复请求
